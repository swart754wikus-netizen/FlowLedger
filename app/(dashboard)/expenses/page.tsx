...
import { doc, collection, setDoc, addDoc, updateDoc, query, where, getDocs } from 'firebase/firestore';
...
import { Receipt, X, Loader2, Pencil } from 'lucide-react';
import type { VatTreatment, Expense } from '@/types/domain';
...
const EMPTY_FORM = { category: 'Materials', supplierName: '', date: new Date().toISOString().slice(0,10), amount: '', vatTreatment: 'inclusive' as VatTreatment, notes: '' };
...
export default function ExpensesPage() {
  ...
  function openEdit(e: Expense) {
    setForm({ category: e.category, supplierName: e.supplierName || '', date: e.date, amount: String(e.amount), vatTreatment: e.vatTreatment, notes: e.notes || '' });
    setEditingId(e.id);
    setShowModal(true);
  }
  ...
  async function save() {
    ...
    try {
      if (editingId) {
        await updateDoc(doc(db, 'expenses', editingId), { category: form.category, supplierName: form.supplierName || null,
          date: form.date, amount, vatTreatment: form.vatTreatment, vatAmount, notes: form.notes });
        const txSnap = await getDocs(query(collection(db, 'transactions'), where('sourceType', '==', 'expense'), where('sourceId', '==', editingId)));
        if (!txSnap.empty) {
          await updateDoc(txSnap.docs[0].ref, { date: form.date, amount, vatAmount, reference: form.supplierName || form.category, category: form.category });
        }
        ...
        <div className="space-y-2">
          {expenses.map(e => (
            <div key={e.id} className="glass flex items-center gap-4 rounded-xl p-4">
              ...
              <button onClick={() => openEdit(e)} className="flex h-7 w-7 items-center justify-center rounded-lg text-t3 hover:bg-midnight-raised hover:text-t1"><Pencil className="h-3.5 w-3.5" /></button>
            </div>
          ))}
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-midnight/85 backdrop-blur-sm p-4">
          <div className="glass-raised w-full max-w-md rounded-2xl p-6">
            <div className="mb-4 flex items-center justify-between">
              <h3 className="text-[16px] font-semibold text-t1">{editingId ? 'Edit Expense' : 'New Expense'}</h3>
              <button onClick={closeModal} className="text-t2 hover:text-t1"><X className="h-4 w-4" /></button>
            </div>
            <div className="space-y-3">
              ...
              <button onClick={save} disabled={saving} className="flex w-full items-center justify-center gap-2 rounded-xl bg-emerald py-2.5 text-[13px] font-semibold text-midnight disabled:opacity-60">
                {saving && <Loader2 className="h-4 w-4 animate-spin" />} {editingId ? 'Update expense' : 'Save expense'}
              </button>
            </div>
          </div>
        </div>

...
export default function NewQuotePage() {
  ...
  const [expiryDate, setExpiryDate] = useState(() => { const d = new Date(); d.setDate(d.getDate()+30); return d.toISOString().slice(0,10); });
  ...
  useEffect(() => {
    if (!editId) return;
    (async () => {
      const snap = await getDoc(doc(db, 'quotes', editId));
      if (snap.exists()) {
        ...
        setNotes(q.notes || '');
        setLines(q.lineItems?.length ? q.lineItems : [newLine()]);
      }
  ...
  async function handleUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      console.log('Extracted quote data:', ex);
      ...
      if (ex.dueDate) { setExpiryDate(ex.dueDate); appliedSomething = true; }
      ...
      if (appliedSomething) toast('Quote details extracted — please review before saving');
      else toast('Could not find any quote details in that file — try a clearer photo or fill it in manually', 'error');
    } catch (err: any) {
    ...
    try {
      if (editId) {
        await updateDoc(doc(db, 'quotes', editId), {
          ...
          status, issueDate, expiryDate, lineItems: lines.filter(l => l.description),
          discountPct, subtotal: totals.subtotal, vatAmount: totals.vatAmount, total: totals.total,
          notes, updatedAt: Date.now(),
        });
        toast('Quote updated');
      } else {
        const quotesSnap = await getDocs(query(collection(db, 'quotes'), where('businessId', '==', business.id)));
        const quoteNumber = `QUO-${String(quotesSnap.size + 1).padStart(4, '0')}`;

        const ref = doc(collection(db, 'quotes'));
        await setDoc(ref, {
          id: ref.id, businessId: business.id, quoteNumber, customerId: customer.id, customerName: customer.name,
          status, issueDate, expiryDate, lineItems: lines.filter(l => l.description),
          discountPct, subtotal: totals.subtotal, vatAmount: totals.vatAmount, total: totals.total,
  ...
  return (
    <div className="max-w-2xl space-y-6">
      {!editId && (
        <>
          <button type="button" onClick={() => fileInputRef.current?.click()} disabled={extracting}
            ...
            {extracting ? 'Reading document…' : 'Upload a quote photo or PDF to auto-fill'}
          </button>
      ...
      <div className="glass rounded-2xl p-5 space-y-4">
        <h2 className="text-[14px] font-semibold text-t1">{editId ? 'Edit quote' : 'Quote details'}</h2>
        <div>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div><label className="mb-1.5 block text-[12px] text-t2">Expires</label>
            <input type="date" className={INP} value={expiryDate} onChange={e => setExpiryDate(e.target.value)} /></div>
        </div>
      </div>
      ...
      <div className="glass rounded-2xl p-5">
        ...
        <textarea className={`${INP} min-h-[70px] resize-none`} value={notes} onChange={e => setNotes(e.target.value)} placeholder="Terms, validity conditions…" />
      </div>

      <div className="flex gap-3">
        <button onClick={() => save('draft')} disabled={saving} className="flex-1 flex items-center justify-center gap-2 rounded-xl border border-midnight-border2 py-3 text-[14px] text-t1 hover:bg-midnight-raised disabled:opacity-60">
          {saving && <Loader2 className="h-4 w-4 animate-spin" />} Save as Draft
        </button>
        <button onClick={() => save('sent')} disabled={saving} className="flex-1 rounded-xl bg-emerald py-3 text-[14px] font-semibold text-midnight hover:brightness-110 disabled:opacity-60">
          {editId ? 'Save & Send' : 'Send Quote'}
        </button>
      </div>
    </div>
  );
